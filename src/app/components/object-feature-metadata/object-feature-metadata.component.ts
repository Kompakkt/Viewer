...
export class EntityFeatureMetadataComponent implements OnInit {
  ...
  public baseURL =
    'https://blacklodge.hki.uni-koeln.de/builds/EntitiesRepository/live/#/entity-overview?entity=';
        ...
        if (this.actualMetadata_id !== currentEntity.relatedDigitalEntity._id) {
          ...
          if (this.actualMetadata_id && this.actualMetadata_id !== '') {
            this.metadata = await this.metadataService.fetchMetadata(
              this.actualMetadata_id,
            );
            this.showMetadata = this.metadata && this.metadata !== '';
            this.isExternal = currentEntity.dataSource.isExternal;
            this.service =
              this.isExternal && currentEntity.dataSource.service
                ? currentEntity.dataSource.service
                : 'Entities Repository';
          } else {
  ...
  public generateDownloadJsonUri(value) {
    ...
    const URI = this.sanitizer.bypassSecurityTrustUrl(
      `data:text/json;charset=UTF-8,${encodeURIComponent(product)}`,
    );
