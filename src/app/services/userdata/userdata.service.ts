...
export class UserdataService {
  ...
  constructor(
    ...
    this.processingService.Observables.actualCollection.subscribe(
      actualCollection => {
        if (
          actualCollection &&
          actualCollection._id &&
          actualCollection.relatedOwner
        ) {
          this.isCollectionOwner =
            isLDAPUser(this.currentUserData) &&
            this.currentUserData.data.compilation
              ? (this.currentUserData.data.compilation.filter(
                  comp => comp,
                ) as ICompilation[]).find(
                  comp => comp._id === actualCollection._id,
                ) !== undefined
              : false;
        } else {
        ...
        .then(userData => {
          ...
          if (userData && userData.message === 'Invalid session') {
            ...
            if (userData.data.compilation) {
              (userData.data.compilation.filter(
                comp => comp,
              ) as ICompilation[]).forEach(compilation => {
                if (
                  !this.personalCollections.find(
                    _c => _c._id === compilation._id,
                  )
                ) {
            ...
            if (userData.data.annotation) {
              (userData.data.annotation.filter(
                ann => ann,
              ) as IAnnotation[]).forEach(annotation => {
                if (
                  !this.userOwnedAnnotations.find(
                    _a => _a._id === annotation._id,
                  )
                ) {
          ...
          console.log(
            'Meine Entityle',
            this.userOwnedFinishedEntities,
            'compis',
            this.personalCollections,
            'meine Annos',
            this.userOwnedAnnotations,
          );
        })
        .catch(error => {
          console.error(error);
  ...
  public isAnnotationOwner(annotation: IAnnotation): boolean {
    return annotation._id &&
      this.loggedIn &&
      this.currentUserData._id !== 'guest'
      ? this.currentUserData._id === annotation.creator._id
      : false;
  }
